...
vfile = open("graph/v.csv", "w", encoding="utf-8") # aid
for n in v:
    vfile.write(n + "\n")
vfile.close()


efile = open("graph/e.csv", "w", encoding="utf-8") # første kolonne aid, følgende er hvem de har spilt med
for k in e:
    efile.write(k + ",")
    for x in e[k]:
        efile.write(x + ",")
...
wfile = open("graph/w.csv", "w", encoding="utf-8") # første og andre kolonne er tuppel fulgt av film og vekt
for x in w:
